...
### 19.7 真实的TLB条目
最后，让我们简单的看看真实的TLB。这个例子来自MIPS R4000，一个使用软件管理TLB的现代系统；一个稍微简化的MIPS TLB条目如图19_4。

![图19_4 "一个MIPS TLB条目"](Figure19_4.png "一个MIPS TLB条目")

MIPS R4000支持页大，小为4KB的32位地址空间。因此，我们希望在我们典型的虚拟地址中有20位VPN和12位偏移量。然而，正如你从TLB中看到的那样，只有19位的VPN，正如它产生的，用户地址将只来自地址空间的一半(剩下的留给kernel)因此，VPN只需要19位。VPN转换为24位的物理帧号(PFN)，因此，可以支持系统到64GB的(物理)主内存($2^24$ 4KB页)。

在MIPS的TLB中还有其他有趣的位。我们可以看到一个 _全局global_ 位(G)，用于那些进程间全局共享的页。因此，如果全局位被设置了，ASID就会被忽略。我们也看到一个8位ASID，OS用它可以区分不同的地址空间(就像上面描述的那样)。有个问题给你：如果在同一个时刻超过了256($2^8$)个进程运行，OS会怎么做？最后，我们看到3个 _Coherence_ (C)位，它决定了硬件是如何缓存一个页(有点超过本章的范围)；一个 _脏检查 dirty_ 位，当这个页被写入数据后会被标记(稍后可以看到它的作用)；一个 _有效valid_ 位，告诉硬件这个条目中是否存在一个有用的转换。还有一个 _页掩码 page mask_ 域(这里没有显示)，它支持多个页表大小；我们稍后将看到有一个更大的页可能很有用。最后，还有一些位没有使用(图中是灰色的)。

MIPS的TLB通常有32个或者64个条目，大部分是当用户进程运行时使用的。然而，有一些被保留供OS使用。一个 _wired_ 位，由OS设置，告诉硬件TLB中有多少个槽是保留给OS使用的；OS使用这些保留映射槽映射到哪些OS想在关键时刻访问的代码和数据，这里，如果一个TLB未命中会导致问题(例如，在TLB miss handler中)。

因为MIPS的TLB是软件管理的，需要指令更新TLB。MIPS提供了4个指令:`TLBP`,用来探测TLB，查找是否有特定的转换在里面；`TLBR`,把TLB条目的内容读入到寄存器中，`RLBWI`，用来替换特定的TLB条目；以及`TLBWR`,用来随机替换一个TLB条目。OS使用这些指令管理TLB内容。这些指令都是 __特权__ 指令；想象一下如果一个用户进程可以修改TLB的内容(提示：任何事，包括控制机器，运行它自己的恶意"OS"，甚至让太阳消失)。

###19.8 总结
我们看到了硬件如何帮助我们让地址转换变得更快。通过提供一个小的，专用的片上TLB作为地址转换缓存，大多数内存应用将希望本缓存而不需要访问主存页表。因此，通常的情况，程序的性能很接近内存没有被完全虚拟化，对于操作系统是一个卓越成就，也肯定是基本要素对于要使用分页的现代系统。

然而，TLB没有让每个存在的程序的世界都没好。具体来说，如果一个程序在短期内访问的页数量超过填入到TLB中页数量，程序将会产生巨大数量的TLB未命中，从而要比平常更慢一些。我们称这个现象叫做超过了 __TLB覆盖 TLB coverage__，对于某些程序来说这是一个问题。一个解决办法，我们将会在下一章讨论，包括支持更大的页表尺寸；通过映射关键的数据结构到程序的地址空间中那些被映射到更大页表的区域中，TLB的有效覆盖率可以增加。对大页表的支持通常由类似 __数据管理系统database management system(DBMS)__ 利用，这些系统的某些数据结构通常很大，还需要随机访问。

>### TIP:RAM不总是RAM(CULLER定律)
>词语 __随机访问内存__，简称 __RAM__,暗示你可以访问RAM中任意部分速度都和其他部分一样快。通常，这样想RAM是好的，因为硬件/OS的功能，例如TLB，访问内存特定的页可能耗费很高，特别是如果二这个页当前没有映射到你的TLB中。因此，记住这个实现tip总是好的：__RAM不总是RAM__。有时候，随机访问你的地址空间，特别是访问那些超过TLB覆盖的页表，可能会导致严重的性能惩罚。因为我们的一个顾问，David Culler，曾经经常指出TLB是很多性能问题的来源，我们以他命名这个定律：__Culler's Law__。

另一个TLB问题值得提示一下：TLB访问可以很容易的变成CPU管道的瓶颈，特别的，一个被叫做 __索引的物理缓存physically-indexed cache__。因为这个缓存，地址转换不得不发生在缓存被访问前，这可能会导致事情变得很慢。由于这个潜在的问题，人们观察了各种聪明的方法访问有着 _虚拟_ 地址的缓存，这避免了昂贵的转换步骤，万一缓存命中，例如，__索引的虚拟缓存 virtually-indexed 缓存__解决了某些性能问题，但是引入了新出的问题到硬件设计中。
