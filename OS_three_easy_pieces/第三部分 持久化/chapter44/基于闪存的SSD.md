## 基于闪存的SSD
在硬盘驱动主宰了数十年后，一种新型持久化存储设备最近在世界范围取得了重要意义。通常称它为 __固态硬盘(solid-state storage)__，这种设备不像硬盘那样有机械或者移动部分；它们是由晶体管(transistors)构建的，很像内存和处理器。然而，不像典型的随机访问内存(例如，DRAM)，这样一个 __固态存储设备(solid-state storage device)__(也即，__SSD__)在掉电时也会保持数据，从而是数据持久化设备的理想候选。
...
>#### 症结：如何构建闪存SSD
>我们要怎样才能构建闪存SSD？我们如何处理擦除这一高开销特性？我们要怎样构建一个可用时间很久的设备，即使重复覆写会导致设备耗尽？技术进步的比赛会停止么？或者会停止在不再让人惊喜？
### 44.1 存储单个bit
闪存芯片设计为在单个晶体管可以存储一到多个bits；晶体管束缚的电荷水平 (the level of charge)被映射到二进制值(the level of charge trapped within the transistor is mapped to binary value)。在 __单层cell(single-level cell SLC)__ 闪存，一个晶体管只存储一个bit(1或者0)；而在 __多级cell(multi-level cell MLC)__ 闪存中，两位被编码为不同的电荷水平，例如，00，01，10和11分别代表，低，稍微低，稍微高和高水平。甚至还有 __三层cell(triple-level cell TLC)__ 闪存，在每个cell中编码了三个位。总之，SLC芯片性能更高但是也更贵。
...
>#### tip:对术语要仔细
>你可能注意到了我们在闪存这个上下文中使用的一些术语在之前被使用过很多次(例如blocks，pages)，但是和之前有些许的差别。没有创建新词语会让你的生活更难(尽管它们可能会这样做)，但是却没有术语决定权的权威机构。对你来说block可能是一个page，但是对别人可能是别的东西，反之亦然，这依赖于上下文。你的工作很简单：了解每个领域中合适的术语，使用它们，这样熟悉这个领域的人(people well-versed in the discipline)就可以理解你在讨论什么。这就是那种时刻，唯一的方法很简单，但是痛苦：使用你的记忆力。
...
* __读(一个page)：__ 闪存芯片客户可以读取任意page(例如，2KB或者4KB)，只需要向设备指定读命令和合适的page号(page number)。这个操作很快，大约10几微秒，与期在设备上的位置无关，也(多少)和之前请求位置无关(和硬盘很不像)。访问任何位置都一样快意味着设备是一个 __随机访问(random access)__ 设备。
* __擦除(一个block)：__ 闪存在写入一个 _page_ 前，设备的特点要求你首先要 __擦除(erase)__ page所在的整个block。重要的是，擦除会毁坏block的内容(通过设置每个bit的值为1)；因此，你必须保证在执行擦除前，这个block中你关心的任何数据已被复制到其它地方了(内存，或者另一个闪存block)。擦除命令很费时间，大概几毫秒才能完成。一旦结束，整个block被重置每个页都准备好被编程了。
* __编程(一个页)：__ 一旦一个block被擦除了，编程命令可以用来修改page内某些1为0，从而把希望的page内容写入到闪存中。编程一个page比擦除一个block开销少，但是比读取一个page开销多很多，在现代闪存芯片上通常花费100多微妙。

一个看待闪存芯片的方式是每个page都有一个相关的状态。page开始是`INVALID`状态。通过擦除page所在的block，就可以设置page的状态(以及block内所有page的状态)为`ERASED`，这重置了block内每个页的内容，也(很重要的)让这些page可编程了。当你编程了一个page，它的状态修改为了`VALID`，意味着设置了它的内容并可以读取了。读取不会影响这些状态(尽管你应该只从已编程过的页读取内容)。一旦一个页被编程了，只有通过擦除这个page位于的整个block才能修改它的内容。这里有一个包含4个页的block经历各种擦除和编程操作后状态转换例子：
...
写一个页就很麻烦；整个block必须先要擦除(先小心移动任何我们关心的数据到别的位置)，然后需要的page就可被编程了。这个不仅仅开销很高，频繁重复这个编程/擦除循环会导致闪存芯片最大的可靠性问题：__耗尽(wear out)__。当设计一个使用闪存的存储系统时，写入的性能和可靠性是关注中心。我们很快会学到现代SSD是如何解决这些问题的，尽管存在这些限制，还是提供了极好的性能和可靠性。
### 44.4 闪存性能和可靠性
...
### 44.10 耗散对齐(wear leveling)
最后，现代FTL必须实现的一个相关后台活动是 __耗散对齐(wear leveling)__，之前介绍过。基本概念很简单：因为多次擦除/编程周期会耗尽一个闪存block，FTL必须要尽可能把工作平均分布到设备所有block上。这种方式，所有block会大致在同样的时间耗尽，而不会让少数常用的block很快的变得不可用。

基本的日志结构方式在分散工作集的初始工作的很好，垃圾换收集也会有帮助。然而，有时候，一个block可能会被长期活跃的数据所填充，而且不会被覆写；在这种情况下，垃圾收集永远不会回收这个block，因此它就不会公平共享工作集(it does not receive its fair share of the write load)。

为了解决这个问题，FTL必须周期的从这些block中读取所有活跃的数据然后重写到其它位置，让这些block再次可用。这个耗尽对齐的过程增加了SSD的写放大，并且由于保证所有block耗尽时间大致相同而要求额外的I/O降低了性能。

### 44.11 SSD性能和开销
和硬盘不同，闪存SSD没有机械模块，事实上它在多方面上类似DRAM，因为它们是"随机访问"设备。在性能中，和硬盘相比较，SSD最大的不同出现在随机读写；一个典型硬盘驱动每秒只能执行几百个随机I/O，SSD可以做的更好。这里，我们使用来着SSD的数据看一下SSD可以执行的有多好；我们特别感兴趣的是FTL对原始芯片性能问题隐藏的有多好。

图44_4显示了三种不同SSD的性能数据，和一个最顶级的硬盘；这个数据来自几个不同在线源。左边两列显示了随机I/O性能，右边两列是顺序I/O；开始的三行显示了三个不同SSD的数据(三星，希捷和英特尔)，最后一行是 __硬盘驱动(hard disk drive)__(简称 __HDD__) 的性能数据，这里是一个希捷高端驱动。

从这个表中我们可以看到一些有意思的事情。首先，最戏剧化的是，这些SSD和单个硬盘之间随机I/O性能区别。SSD获得了几十甚至数百MB/s随机I/Os，这个"高性能"硬盘峰值只有几MB/s(事实上，我们取证到2MB/s)。第二，按照顺序性能，差异就很小了；SSD虽然执行的很好，如果你只需要顺序性能，SSD依旧是很好的一个选择。第三，你可以看到SSD随机读取性能没有SSD随机写性能好。SSD意外好的随机写性能源于很多SSD日志结构设计，把随机写转换为顺序写提升性能。最后，由于SSD在顺序和随机I/O之间存在某些性能差异，我们在接下来学习关于如何为硬盘构建文件系统的技术依旧可以应用于SSD上；尽管顺序和随机I/O的差异量级变得更小了，二者之间依旧有足够的差距要仔细考虑如何设计文件系统降低随机I/Os。

#### 花费
正如我们在上面看到的，SSD的性能远超现代硬盘，甚至是在执行顺序I/O。所以，为什么SSD没有完全替换硬盘作为存储材质的选择？答案很简单：花费，跟具体的说，每单元存储花费。当前，一个SSD的花费大概是250GB价格150刀；这样每GB花费60美分。一个典型的硬盘大概1TB花费50美刀，意味着每GB花费5美分。在这两个存储媒介之间价格依旧还有超过10倍的差距。

这种性能和花费差异指示了大规模存储系统要如何构建。如果性能是主要关注点，SSD是一个极好的选择，特别是如果随机性能很重要。另一方面，如果，你正在组建大规模数据中心并且希望存放海量信息，巨大的开销差异会导致你选择硬盘。当然，混合方式很有意义--某些存储系统汇合了SSD和硬盘，对更"热"的数据使用较少量的SSD提供高性能，在硬盘上存储剩下"较冷的"(较少使用的)数据以节省花费。只要价格差异存在，硬盘就会存在。

### 44.12 总结
闪存SSD在笔记本，台式机，驱动了全球经济的数据中心服务器中正变得常见。因此，你应该要知道它们的一些细节，对吧？

这里有一个坏消息：本章(就想这本书的很多章)只是理解现状的第一步。你还要在其他地方获取更多的信息：原始技术，包括关于实际设备性能的研究，FTL设计的问题，甚至是由闪存组成的分布式系统。为了从一个SSD中榨取高性能，你需要的所有事情，都可以在一个关于SSD"不成文的约定(unwritten contract)"的论文中找到很好的概览(HK+17 The Unwritten Contract of Solid State Drives)

不要仅仅只阅读学术论文，还要在阅读流行期刊中阅读最近的进展。在那里你可以学到更实际(但是依旧有用的)信息，例如，三星在同一块SSD同时利用了TLC和SLC cell来最大化性能(SLC可以快速缓冲写)和容量(TLC可以在每个cell中存储更多bit)。也就是，就像他们说的，只是冰山一角。深入研究这种研究冰山，可以从Ma和其合作者极好的(同时是最近的)调查开始(M+14 "A Survey of Address Translation Technologies for Flash Memories")。仔细思考，冰山可以撞沉最大的冰山。
>#### 附注：关键SSD词语
>* 一个 __闪存芯片(flash chip)__ 由很多bank组成，每个bank由多个 __擦除blocks(erase blocks)__ 组织(有时候也叫做 __blocks__)。每个block进一步分成几个 __page__。
>* block很大(128KB-2MB)包含了很多pages，这个page相对很小(1KB-8KB)
>* 为了从flash读取，发起一个包含地址和长度的读取命令；这允许客户端读取一个或者多个pages。
>* 写flash更复杂。首先，客户端必须要 __擦除(erase)__ 整个block(会删除这个block内所有信息)。然后，客户端可以 __编程__ 每个page恰好一次，从而完成了写。
>* 一个新的 __trim__ 操作很有用，用来告诉设备当一个特定block(或者一个范围内的block)不在需要。
>* 闪存可靠性 __耗尽(wear out)__ 具有决定性；如果一个block被擦除然后别编程太频繁，他就会变得不可用。
>* 基于闪存的 __固态存储设备(solid-state storage device SSD)__ 行为就好像他是一个普通基于block的读写硬盘；通过使用一个 __闪存转换层(flash translation layer FTL)__，它把来自客户端的读写转换为底层闪存芯片的读，擦除和编程。
>* 大多数FTL是 __日志结构的(log-structured)__，通过最小化擦除/编程周期减少了写入开销。一个内存转换层跟踪了逻辑写在物理材质的位置。
>* 日志结构FTL的一个关键问题是 __垃圾收集(garbage collection)__ 的开销，这会导致 __写放大(write amplification)__。
>* 另一个问题是映射表大小，可能会变得很大。使用 __混合映射(hybrid mapping)__ 或者只 __缓存(caching)__ FTL的热数据有可能解决。
>* 最后一个是 __消耗对齐(wear leveling)__；为了保证所说block接受它们擦除/编程负载分担，FTL也会必须要偶尔从最常读取的block中汇合数据。